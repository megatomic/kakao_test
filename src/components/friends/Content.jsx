...
const Content = ({ search, showProfile }) => {
  ...
  const friendsList = [
    { id: 0, name: '손흥민', statusMsg: '월드컵16강 가자' },
    { id: 1, name: '김민재', statusMsg: '나이만 먹는다' },
    { id: 2, name: '황희찬', statusMsg: '울버햄튼 망해라' },
  ];
  ...
  const renderFriends = searchedFriends.map((friend) => {
    return <FriendRowCell key={friend.id} name={friend.name} statusMsg={friend.statusMsg} profileImgClick={() => showProfile(friend)} onDoubleClick={() => {}} />;
  });
      ...
      {search ? null : (
        <div>
          <MyProfileBlock onDoubleClick={onMyBlockDoubleClick}>
            <img src="/asset/base_profile.jpg" alt="profile_image" onClick={() => showProfile({ id: 0, name: profileName, statusMsg: stateMsg })} />
            <div className="profile">
              <p className="profileName">
                <b>{profileName}</b>
              </p>
              <p className="profileMsg">{stateMsg}</p>
            </div>
          </MyProfileBlock>
        </div>
      )}
